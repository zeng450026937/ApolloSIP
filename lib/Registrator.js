...
const debug = require('debug')('SIP:Registrator');
...
${this._contact};expires=${this._expires}${this._extraContactParams}`);
    ...
    const request = new SIPMessage.OutgoingRequest(
      SIP_C.REGISTER, this._registrar, this._ua, {
        'to_uri'  : this._to_uri,
    ...
    const request_sender = new RequestSender(this._ua, request, {
      onRequestTimeout : () =>
      {
        this._registrationFailure(null, SIP_C.causes.REQUEST_TIMEOUT);
      },
      onTransportError : () =>
      {
        this._registrationFailure(null, SIP_C.causes.CONNECTION_ERROR);
      },
        ...
        switch (true)
        {
          ...
          case /^423$/.test(response.status_code):
            ...
            { // This response MUST contain a Min-Expires header field
              ...
              this._registrationFailure(response, SIP_C.causes.SIP_FAILURE_CODE);
            }
            break;
          default:
          {
  ...
  unregister(options = {})
  {
    ...
    const request_sender = new RequestSender(this._ua, request, {
      onRequestTimeout : () =>
      {
        this._unregistered(null, SIP_C.causes.REQUEST_TIMEOUT);
      },
      onTransportError : () =>
      {
        this._unregistered(null, SIP_C.causes.CONNECTION_ERROR);
      },
      // Increase the CSeq on authentication.
      onAuthenticated : () =>
      {
