...
const debug = require('debug')('SIP:RequestSender');
...
module.exports = class RequestSender
{
  constructor(ua, request, eventHandlers)
  {
    ...
    if (ua.status === ua.C.STATUS_USER_CLOSED &&
        (this._method !== SIP_C.BYE || this._method !== SIP_C.ACK))
    {
      this._eventHandlers.onTransportError();
    }

    // If ua has credentials cache, authenticate the request.
    const credential = ua._cache.credentials[this._ua.configuration.realm];
    const challenge = ua._cache.challenges[this._ua.configuration.realm];

    if (credential && challenge) 
    {        
      credential.authenticate(this._request, challenge);
      this._request.setHeader('authorization', credential.toString());
      this._request.setHeader('proxy-authorization', credential.toString());
    }
  }
    ...
    */
        ...
        (this._ua.configuration.password !== null || this._ua.configuration.ha1 !== null))
    {
      ...
      if (!this._challenged || (!this._staled && challenge.stale === true))
      {
        ...
        // Cache credentials in th UA.
        this._ua._cache.credentials[this._ua.configuration.realm] = this._auth;
        this._ua._cache.challenges[this._ua.configuration.realm] = challenge;
