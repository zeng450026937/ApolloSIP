...
const debug = require('debug')('SIP:RTCSession');
const debugerror = require('debug')('SIP:ERROR:RTCSession');
...
module.exports = class RTCSession extends EventEmitter
{
  ...
  init_incoming(request, initCallback)
  {
    ...
    {
      request.reply(408);
      this._failed('local', null, SIP_C.causes.NO_ANSWER);
    ...
    if (expires)
    {
      this._timers.expiresTimer = setTimeout(() =>
      {
        ...
        {
          request.reply(487);
          this._failed('system', null, SIP_C.causes.EXPIRES);
        }
      }, expires
      );
    }
        ...
        else if (mediaConstraints.audio || mediaConstraints.video)
        {
          ...
          return navigator.mediaDevices.getUserMedia(mediaConstraints)
            .catch((error) =>
            {
              ...
              request.reply(480);
              this._failed('local', null, SIP_C.causes.USER_DENIED_MEDIA_ACCESS);
      ...
      .then(() =>
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          .catch((error) =>
          {
            ...
            this._failed('system', null, SIP_C.causes.WEBRTC_ERROR);
  ...
  terminate(options = {})
  {
    ...
    const cause = options.cause || SIP_C.causes.BYE;
    const extraHeaders = Utils.cloneArray(options.extraHeaders);
    const body = options.body;
    const eventHandlers = {
      onSuccessResponse : () => 
      {
        this._finished('local', null, cause);
      }
    };
  ...
  _setACKTimer()
  {
    this._timers.ackTimer = setTimeout(() =>
    {
      if (this._status === C.STATUS_WAITING_FOR_ACK)
      {
        ...
        clearTimeout(this._timers.invite2xxTimer);
        this.sendRequest(SIP_C.BYE);
        this._ended('remote', null, SIP_C.causes.NO_ACK);
      }
    }, Timers.TIMER_H);
  }
  ...
  _createLocalDescription(type, constraints)
  {
    ...
    return Promise.resolve()
      ...
      .then((desc) =>
      {
        const e = { originator: 'local', type: type, desc: desc };

        this.emit('desc', e);

        return connection.setLocalDescription(e.desc)
          .catch((error) =>
          {
            this._rtcReady = true;
  ...
  _processInDialogSdpOffer(request)
  {
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() =>
      {
        ...
        // we don't want remoteHold
        // and always show local stream
        /*
    ...
    Promise.resolve()
      ...
      .then((stream) =>
      {
        ...
        return this._createLocalDescription('offer', rtcOfferConstraints)
          .catch(() =>
          {
            this._failed('local', null, SIP_C.causes.WEBRTC_ERROR);
            ...
            if (this._connection.signalingState === 'stable')
            {
              return this._connection.createOffer()
                ...
                .catch((error) =>
                {
                  ...
                  this._failed('local', response, SIP_C.causes.WEBRTC_ERROR);
                });
            }
          })
          .then(() =>
  ...
  _handleSessionTimersInIncomingResponse(response)
  {
    ...
    if (response.session_expires &&
        response.session_expires >= SIP_C.MIN_SESSION_EXPIRES)
    {
  ...
  _runSessionTimer()
  {
    ...
    if (this._sessionTimers.refresher)
    {
      this._sessionTimers.timer = setTimeout(() =>
      {
        ...
        if (this._sessionTimers.refreshMethod === SIP_C.UPDATE)
        {
          this._sendUpdate();
        }
        else
  ...
  _finished(originator, message, cause)
  {
