...
const debug = require('debug')('SIP:UA');
const debugerror = require('debug')('SIP:ERROR:UA');
...
module.exports = class UA extends EventEmitter
{
  ...
  sendService(target, event, body, options)
  {
    debug('sendService()');
  ...
  terminateSubscriptions()
  {
    ...
    for (const subscription in this._subscriptions)
    {
      if (Object.prototype.hasOwnProperty.call(this._subscriptions, subscription))
      {
        ...
        try { this._subscriptions[subscription].terminate(); }
        catch (error) {}
      }
    }
  }
   ...
   */
  _findSubscription(request)
  {
    const call_id = request.call_id;
    const from_tag = request.from_tag;
    const to_tag = request.to_tag;
    const event = request.getHeader('event');

    const subscriptionIDa = call_id + from_tag + event;
    const subscriptionA = this._subscriptions[subscriptionIDa];
    const subscriptionIDb = call_id + to_tag + event;
