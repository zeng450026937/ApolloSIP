...
const debug = require('debug')('SIP:Dialog');
...
module.exports = class Dialog
{
  ...
  _createRequest(method, extraHeaders, body)
  {
    ...
    const cseq = (method === SIP_C.CANCEL || method === SIP_C.ACK) ?
      this._local_seqnum :
      this._local_seqnum += 1;
  ...
  _checkInDialogRequest(request)
  {
    ...
    if (request.method === SIP_C.INVITE ||
        (request.method === SIP_C.UPDATE && request.body))
    {
