...
const debug = require('debug')('SIP:RTCSession:ReferSubscriber');
...
module.exports = class ReferSubscriber extends EventEmitter
{
  ...
  sendRefer(target, options = {})
  {
    ...
    const request = this._session.sendRequest(SIP_C.REFER, {
      extraHeaders,
      eventHandlers : {
        ...
        {
          this._requestFailed(response, SIP_C.causes.REJECTED);
        },
        onTransportError : () =>
        {
          this._requestFailed(null, SIP_C.causes.CONNECTION_ERROR);
        },
        onRequestTimeout : () =>
        {
          this._requestFailed(null, SIP_C.causes.REQUEST_TIMEOUT);
        },
        onDialogError : () =>
        {
          this._requestFailed(null, SIP_C.causes.DIALOG_ERROR);
        }
      }
    });
